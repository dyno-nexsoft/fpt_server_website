...
///
/// [source] matches `admin.logs.tail`'s own parameter (`server` or `novnc`)
/// and rides along as a query parameter, so the viewer is bookmarkable per
/// log. A second route and a second screen would have differed only in which
/// string they passed through.
class ServerLogsRoute extends GoRouteData with $ServerLogsRoute {
  const ServerLogsRoute({this.source});

  final String? source;

  @override
  Widget build(BuildContext context, GoRouterState state) =>
      ServerLogsScreen(source: source ?? 'server');
}
